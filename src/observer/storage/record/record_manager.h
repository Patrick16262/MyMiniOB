
/* Copyright (c) 2021 Xie Meiyi(xiemeiyi@hust.edu.cn) and OceanBase and/or its affiliates. All rights reserved.
miniob is licensed under Mulan PSL v2.
You can use this software according to the terms and conditions of the Mulan PSL v2.
You may obtain a copy of Mulan PSL v2 at:
         http://license.coscl.org.cn/MulanPSL2
THIS SOFTWARE IS PROVIDED ON AN "AS IS" BASIS, WITHOUT WARRANTIES OF ANY KIND,
EITHER EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO NON-INFRINGEMENT,
MERCHANTABILITY OR FIT FOR A PARTICULAR PURPOSE.
See the Mulan PSL v2 for more details. */

//
// Created by Meiyi & Longda on 2021/4/13.
//
#pragma once

#include <limits>
#include <sstream>

#include "common/lang/bitmap.h"
#include "storage/buffer/disk_buffer_pool.h"
#include "storage/record/record.h"
#include "storage/record/record_log.h"
#include "common/types.h"

class LogHandler;

class ConditionFilter;

class RecordPageHandler;

class LogHandler;

class Trx;

class Table;

/**
 * @defgroup RecordManager
 *
 * 问题1：那么如果记录不是定长的，还能使用 slot num 吗？ 答：能，但是效率低
 * 问题2：如何更有效地存放不定长数据呢？ 答：多用几个slot
 * 问题3：如果一个页面不能存放一个记录，那么怎么组织记录存放效果更好呢？ 答：多用几个页面
 */

/**
 * @ingroup RecordManager
 * @details 每一页都有一个这样的页头
 */
struct PageHeader {
    int32_t record_num;           ///< 当前页面记录的个数
    int32_t record_real_size;     ///< 每条记录的实际大小
    int32_t record_size;          ///< 每条记录占用实际空间大小(可能对齐)
    int32_t record_capacity;      ///< 最大记录个数
    int32_t first_record_offset;  ///< 第一条记录的偏移量

    std::string to_string() const;
};


/**
 * 对record增删查的接口
 * 当前仅在此文件中使用
 */
class RecordHandler {
public:
    virtual RC insert_record(Record &record) = 0;

    virtual RC get_record(const RID &rid, Record &record) = 0;

    virtual RC delete_record(const RID *rid) = 0;

    virtual ~RecordHandler() {

    }
};


/**
 * @brief 负责处理一个页面中各种操作，比如插入记录、删除记录或者查找记录
 *
 * @ingroup RecordManager
 * @details 当前定长记录模式下每个页面的组织大概是这样的：
 * @code
 * | PageHeader | record allocate bitmap |
 * |------------|------------------------|
 * | record1 | record2 | ..... | recordN |
 * @endcode
 */
class RecordPageHandler {
public:
    RecordPageHandler() = default;

    ~RecordPageHandler();

    /**
     * @brief 初始化
     *
     * @param buffer_pool 关联某个文件时，都通过buffer pool来做读写文件
     * @param page_num    当前处理哪个页面
     * @param mode        是否只读。在访问页面时，需要对页面加锁
     */
    RC init(DiskBufferPool &buffer_pool, PageNum page_num, ReadWriteMode mode);

    /**
     * @brief 数据库恢复时，与普通的运行场景有所不同，不做任何并发操作，也不需要加锁
     *
     * @param buffer_pool 关联某个文件时，都通过buffer pool来做读写文件
     * @param page_num    操作的页面编号
     */
    RC recover_init(DiskBufferPool &buffer_pool, PageNum page_num);

    /**
     * @brief 对一个新的页面做初始化，初始化关于该页面记录信息的页头PageHeader
     *
     * @param buffer_pool 关联某个文件时，都通过buffer pool来做读写文件
     * @param page_num    当前处理哪个页面
     * @param record_size 每个记录的大小
     */
    RC init_empty_page(DiskBufferPool &buffer_pool, LogHandler &log_handler, PageNum page_num, int record_size);

    /**
     * @brief 操作结束后做的清理工作，比如释放页面、解锁
     */
    RC cleanup();

    /**
     * @brief 插入一条记录
     *
     * @param data 要插入的记录
     * @param rid  如果插入成功，通过这个参数返回插入的位置
     */
    RC insert_record_part(const char *data, const RID &next_rid, uint8_t record_type, RID *rid);

    /**
     * @brief 数据库恢复时，在指定位置插入数据
     *
     * @param data 要插入的数据行
     * @param rid  插入的位置
     */
    RC recover_insert_record(const char *data, const RID &rid);

    /**
     * @brief 删除指定的记录
     *
     * @param rid 要删除的记录标识
     */
    RC erase_bitmap(const RID *rid);

    /**
     * @brief
     *
     */
    RC update_record_part(const RID &rid, const char *data);

    /**
     * @brief 获取指定位置的记录数据
     *
     * @param rid 指定的位置
     * @param record 返回指定的数据。这里不会将数据复制出来，而是使用指针，所以调用者必须保证数据使用期间受到保护
     * @param next_rid nullable, 返回的下一个record的rid
     */
    RC get_record_part_in_page(const RID &rid, Record &record, RID &next_rid, uint8_t &record_type);

    RC get_record_type(const RID &rid, uint8_t &type);

    /**
     * @brief 返回该记录页的页号
     */
    PageNum get_page_num() const;

    common::Bitmap get_bitmap();

    /**
     * @brief 当前页面是否已经没有空闲位置插入新的记录
     */
    bool is_full() const;

protected:
    /**
     * @details
     * 前面在计算record_capacity时并没有考虑对齐，但第一个record需要8字节对齐
     * 因此按此前计算的record_capacity，最后一个记录的部分数据可能会被挤出页面
     * 所以需要对record_capacity进行修正，保证记录不会溢出
     */
    void fix_record_capacity() {
        int32_t last_record_offset =
                page_header_->first_record_offset + page_header_->record_capacity * page_header_->record_size;
        while (last_record_offset > BP_PAGE_DATA_SIZE) {
            page_header_->record_capacity -= 1;
            last_record_offset -= page_header_->record_size;
        }
    }

    /**
     * @brief 获取指定槽位的记录数据
     *
     * @param 指定的记录槽位
     */
    char *get_record_content(SlotNum slot_num) {
        return frame_->data() + page_header_->first_record_offset + (page_header_->record_size * slot_num);
    }

protected:
    DiskBufferPool *disk_buffer_pool_ = nullptr;  ///< 当前操作的buffer pool(文件)
    Frame *frame_ = nullptr;  ///< 当前操作页面关联的frame(frame的更多概念可以参考buffer pool和frame)
    ReadWriteMode rw_mode_ = ReadWriteMode::READ_ONLY;  ///< 当前的操作是否都是只读的
    PageHeader *page_header_ = nullptr;                    ///< 当前页面上页面头
    char *bitmap_ = nullptr;  ///< 当前页面上record分配状态信息bitmap内存起始位置

};

class RecordNumIterator {
public:
    RecordNumIterator();

    ~RecordNumIterator();

    RC init(DiskBufferPool &bp, PageNum page_num, SlotNum start_slot = 0);

    bool has_next();

    PageNum next();

private:
    void fetch_next_num();

private:
    common::Bitmap bitmap_;
    PageNum page_num_;
    SlotNum next_slot_num = -1;
    RecordPageHandler handler_;
};

/**
 * @brief 管理整个文件中记录的增删改查
 * @ingroup RecordManager
 */
class RecordFileHandler : public RecordHandler {
public:
    RecordFileHandler() = default;

    ~RecordFileHandler();

    /**
     * @brief 初始化
     *
     * @param buffer_pool 当前操作的是哪个文件
     */
    RC init(DiskBufferPool &buffer_pool, LogHandler &log_handler);

    /**
     * @brief 关闭，做一些资源清理的工作
     */
    void close();

    /**
     * @brief 从指定文件中删除指定槽位的记录
     *
     * @param rid 待删除记录的标识符
     */
    RC delete_record(const RID *rid) override;

    /**
     * @brief 插入一个新的记录到指定文件中，并返回该记录的标识符
     *
     * @param record 记录，返回时record中会出现记录的标识符
     */

    RC insert_record(Record &record) override;

    /**
     * @brief 数据库恢复时，在指定文件指定位置插入数据
     * 数据库恢复功能ob暂时还没有，使用该函数会直接报错
     *
     * @param data        记录内容
     * @param record_size 记录大小
     * @param rid         要插入记录的指定标识符
     */
    RC recover_insert_record(const char *data, int record_size, const RID &rid);

    /**
     * update一条record应分为三步
     * 1. 获取
     * 2. 删除
     * 3， 插入
     * @param rid
     * @return
     */
    RC get_record(const RID &rid, Record &record) override;

    /**
     * 检查并更新数据库字段
     * 主要更新的是事务字段
     * @param rid
     * @param updater
     * @return
     */
    RC visit_record(const RID &rid, std::function<bool(Record &)> updater);

    RC get_page_num_iterator(PageNumIterator &iterator);

    RC get_record_iterator(RecordNumIterator &iterator, PageNum page_num);

private:
    /**
     * @brief 插入一个新的记录到指定文件中，并返回该记录的标识符
     *
     * @param data        纪录内容
     * @param data_size 记录大小
     * @param rid         返回该记录的标识符
     */
    RC insert_record_part(const char *data, int data_size,
                          const RID &next_rid, uint8_t record_type, RID *rid);

    RC delete_record_part(const RID *rid);

    RC get_record_part(const RID &rid, Record &record);

    RC get_record_part(const RID &rid, Record &record, RID &next_rid);

    RC get_record_part(const RID &rid, Record &record, RID &next_rid, uint8_t &record_type);
    /**
     * @brief 初始化当前没有填满记录的页面，初始化free_pages_成员
     */
    RC init_free_pages();

private:
    DiskBufferPool *disk_buffer_pool_ = nullptr;  ///资源BufferPool的所有者是table，其余均为借用者
    LogHandler *log_handler_ = nullptr;  ///< 记录日志的处理器
    std::unordered_set<PageNum> free_pages_;                  ///< 没有填充满的页面集合
    std::recursive_mutex lock_;   ///文件锁，每次增删改之前都会加锁，显然这样效率很低,,,
};

/**
 * @brief 遍历某个文件中所有记录
 * @ingroup RecordManager
 * @details 遍历所有的页面，同时访问这些页面中所有的记录
 */
class RecordFileScanner {
public:
    RecordFileScanner() = default;

    ~RecordFileScanner();

    /**
     * @brief 打开一个文件扫描。
     * @details 如果条件不为空，则要对每条记录进行条件比较，只有满足所有条件的记录才被返回
     * @param table            遍历的哪张表
     * @param buffer_pool      访问的文件
     * @param mode             当前是否只读操作。访问数据时，需要对页面加锁。比如
     *                         删除时也需要遍历找到数据，然后删除，这时就需要加写锁
     * @param condition_filter 做一些初步过滤操作
     */
    void open_scan(RecordFileHandler *handler);

    /**
     * @brief 关闭一个文件扫描，释放相应的资源
     */
    void close_scan();

    /**
     * @brief 获取下一条记录
     *
     * @param record 返回的下一条记录
     */
    RC next(Record &record);


private:
    /**
     * @brief 获取该文件中的下一条记录
     */
    RC fetch_next_record();

    /**
     * @brief 获取一个页面内的下一条记录
     */
    RC fetch_record();

private:

    PageNumIterator page_iterator_;
    RecordNumIterator record_iterator_; ///创建record_iterator_会对page加锁
    RecordFileHandler *handler_ = nullptr;
    PageNum page_num_;
    SlotNum slot_num_;
    Record current_record_;                 ///< 获取的记录放在这里缓存起来
};
