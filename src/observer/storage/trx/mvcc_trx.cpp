...
RC MvccTrxKit::init() {
    ...
    fields_ = vector<FieldMeta>(2);
    fields_[0].init("__trx_xid_begin", AttrType::INTS, 0 /*attr_offset*/, 4 /*attr_len*/, false /*visible*/, false, true, false);
    fields_[1].init("__trx_xid_end", AttrType::INTS, 0 /*attr_offset*/, 4 /*attr_len*/, false /*visible*/, false,  true, false);
...
RC MvccTrx::delete_record(Table *table, Record &record) {
    ...
    RC rc = table->visit_record(record.rid(),
                                [this, table, &delete_result, &end_field](Record &inplace_record) -> bool {
...
RC MvccTrx::visit_record(Table *table, Record &record, ReadWriteMode mode) {
    ...
    if (begin_xid > 0 && end_xid > 0) {
        ...
        if (-begin_xid == trx_id_) {
            ...
            LOG_TRACE(
                    "record invisible. someone is updating this record right now. trx id=%d, begin xid=%d, end xid=%d",
        ...
        if (mode == ReadWriteMode::READ_ONLY) {
            ...
            if (-end_xid != trx_id_) {
                LOG_TRACE(
                        "concurrency conflit. someone is deleting this record right now. trx id=%d, begin xid=%d, end xid=%d",
                        trx_id_, begin_xid, end_xid);
