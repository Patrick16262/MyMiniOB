...
int BPFrameManager::purge_frames(int count, function<RC(Frame *frame)> purger) {
    ...
    STD_INFO("purge frames find %ld pages total", frames_can_purge.size());
    ...
    for (Frame *frame: frames_can_purge) {
        ...
        if (RC::SUCCESS == rc) {
            ...
            frame->unpin();
            STD_WARN("failed to purge frame. frame_id=%s, rc=%s",
                     frame->frame_id().to_string().c_str(), strrc(rc));
        }
    }
    STD_INFO("purge frame done. number=%d", freed_count);
    return freed_count;
}
...
////////////////////////////////////////////////////////////////////////////////
PageNumIterator::PageNumIterator() {}

PageNumIterator::~PageNumIterator() {}

RC PageNumIterator::init(DiskBufferPool &bp, PageNum start_page /* = 1 */) {
    ...
    STD_TRACE("inited page num iterator. start page=%d", start_page);
    return RC::SUCCESS;
}

bool PageNumIterator::has_next() {
    return bitmap_.next_setted_bit(current_page_num_ + 1) != -1;
}

PageNum PageNumIterator::next() {
...
DiskBufferPool::~DiskBufferPool() {
    close_file();
    STD_INFO("disk buffer pool exit");
}

RC DiskBufferPool::open_file(const char *file_name) {
    int fd = open(file_name, O_RDWR);
    if (fd < 0) {
        STD_ERROR("Failed to open file %s, because %s.", file_name, strerror(errno));
        return RC::IOERR_ACCESS;
    }
    STD_INFO("Successfully open buffer pool file %s.", file_name);
    ...
    if (ret != 0) {
        STD_ERROR("Failed to read first page of %s, due to %s.", file_name, strerror(errno));
        close(fd);
        file_desc_ = -1;
    ...
    if (rc != RC::SUCCESS) {
        STD_ERROR("failed to allocate frame for header. file name %s", file_name_.c_str());
        close(fd);
        file_desc_ = -1;
        return rc;
    }
    ...
    if ((rc = load_page(BP_HEADER_PAGE, hdr_frame_)) != RC::SUCCESS) {
        STD_ERROR("Failed to load first page of %s, due to %s.", file_name, strerror(errno));
    ...
    STD_INFO("Successfully open %s. file_desc=%d, hdr_frame=%p, file header=%s",
...
RC DiskBufferPool::close_file() {
    ...
    rc = purge_all_pages();
    if (rc != RC::SUCCESS) {
        STD_ERROR("failed to close %s, due to failed to purge pages. rc=%s", file_name_.c_str(), strrc(rc));
        return rc;
    }

    rc = dblwr_manager_.clear_pages(this);
    if (OB_FAIL(rc)) {
        STD_WARN("failed to clear pages in double write buffer. filename=%s, rc=%s", file_name_.c_str(), strrc(rc));
        return rc;
    }
    ...
    if (close(file_desc_) < 0) {
        STD_ERROR("Failed to close fileId:%d, fileName:%s, error:%s", file_desc_, file_name_.c_str(), strerror(errno));
        return RC::IOERR_CLOSE;
    }
    STD_INFO("Successfully close file %d:%s.", file_desc_, file_name_.c_str());
    file_desc_ = -1;
...
RC DiskBufferPool::get_this_page(PageNum page_num, Frame **frame) {
    ...
    *frame = nullptr;
    ...
    if (page_num >= file_header_->page_count) {
        STD_WARN("page %d is not exist. file=%s", page_num, file_name_.c_str());
    ...
    if (rc != RC::SUCCESS) {
        STD_ERROR("Failed to alloc frame %s:%d, due to failed to alloc page.", file_name_.c_str(), page_num);
        return rc;
    }
    ...
    if ((rc = load_page(page_num, allocated_frame)) != RC::SUCCESS) {
        STD_ERROR("Failed to load page %s:%d", file_name_.c_str(), page_num);
...
RC DiskBufferPool::allocate_page(Frame **frame) {
    ...
    if ((file_header_->allocated_pages) < (file_header_->page_count)) {
        ...
        for (int i = 0; i < file_header_->page_count; i++) {
            ...
            if (((file_header_->bitmap[byte]) & (1 << bit)) == 0) {
                ...
                if (OB_FAIL(rc)) {
                    STD_ERROR("Failed to log allocate page %d, rc=%s", i, strrc(rc));
                    // 忽略了错误
                }
    ...
    if (file_header_->page_count >= BPFileHeader::MAX_PAGE_NUM) {
        STD_ERROR("file buffer pool is full. page count %d, max page count %d",
    ...
    if (OB_FAIL(rc)) {
        STD_ERROR("Failed to log allocate page %d, rc=%s", file_header_->page_count, strrc(rc));
        // 忽略了错误
    }
    hdr_frame_->set_lsn(lsn);
    ...
    if ((rc = allocate_frame(page_num, &allocated_frame)) != RC::SUCCESS) {
        STD_ERROR("Failed to allocate frame %s, due to no free page.", file_name_.c_str());
        lock_.unlock();
        return rc;
    }

    STD_INFO("allocate new page. file=%s, pageNum=%d, pin=%d",
    ...
    if ((rc = flush_page_internal(*allocated_frame)) != RC::SUCCESS) {
        STD_WARN("Failed to alloc page %s , due to failed to extend one page.", file_name_.c_str());
...
RC DiskBufferPool::dispose_page(PageNum page_num) {
    if (page_num == 0) {
        STD_ERROR("Failed to dispose page %d, because it is the first page. filename=%s", page_num, file_name_.c_str());
        return RC::INTERNAL;
    }
    ...
    if (used_frame != nullptr) {
        ...
        STD_INFO("page not found in memory while disposing it. pageNum=%d", page_num);
    }

    LSN lsn = 0;
    RC rc = log_handler_.deallocate_page(page_num, lsn);
    if (OB_FAIL(rc)) {
        STD_ERROR("Failed to log deallocate page %d, rc=%s", page_num, strrc(rc));
        // ignore error handle
    }
...
RC DiskBufferPool::purge_frame(PageNum page_num, Frame *buf) {
    if (buf->pin_count() != 1) {
        STD_INFO("Begin to free page %d frame_id=%s, but it's pin count > 1:%d.",
    ...
    if (buf->dirty()) {
        ...
        if (rc != RC::SUCCESS) {
            STD_WARN("Failed to flush page %d frame_id=%s during purge page.", buf->page_num(),
                     buf->frame_id().to_string().c_str());
            return rc;
        }
    }

    STD_INFO("Successfully purge frame =%p, page %d frame_id=%s", buf, buf->page_num(),
              buf->frame_id().to_string().c_str());
...
RC DiskBufferPool::check_all_pages_unpinned() {
    ...
    for (Frame *frame: frames) {
        ...
        if (frame->page_num() == BP_HEADER_PAGE && frame->pin_count() > 1) {
            STD_WARN("This page has been pinned. id=%d, pageNum:%d, pin count=%d",
    ...
    STD_INFO("all pages have been checked of id %d", id());
    return RC::SUCCESS;
}
...
RC DiskBufferPool::flush_page_internal(Frame &frame) {
    ...
    if (OB_FAIL(rc)) {
        STD_ERROR("Failed to log flush frame= %s, rc=%s", frame.to_string().c_str(), strrc(rc));
        // ignore error handle
    }
    ...
    frame.clear_dirty();
    STD_INFO("Flush block. file desc=%d, frame=%s", file_desc_, frame.to_string().c_str());
...
RC DiskBufferPool::flush_all_pages() {
    ...
    for (Frame *frame: used) {
        ...
        frame->unpin();
        if (rc != RC::SUCCESS) {
            STD_WARN("failed to flush all pages");
            return rc;
        }
    }
    return RC::SUCCESS;
...
RC DiskBufferPool::write_page(PageNum page_num, Page &page) {
    ...
    if (lseek(file_desc_, offset, SEEK_SET) == -1) {
        STD_ERROR("Failed to write page %ld of %d due to failed to seek %s.", offset, file_desc_, strerror(errno));
        return RC::IOERR_SEEK;
    }

    if (writen(file_desc_, &page, sizeof(Page)) != 0) {
        STD_ERROR("Failed to write page %ld of %d due to %s.", offset, file_desc_, strerror(errno));
        return RC::IOERR_WRITE;
    }

    STD_INFO("write_page: buffer_pool_id:%d, page_num:%d, lsn=%ld, check_sum=%d", id(), page_num, page.lsn,
              page.check_sum);
    return RC::SUCCESS;
}
...
RC DiskBufferPool::redo_allocate_page(LSN lsn, PageNum page_num) {
    ...
    if (page_num < file_header_->page_count) {
        ...
        if (bitmap.get_bit(page_num)) {
            STD_WARN("page %d has been allocated. file=%s", page_num, file_name_.c_str());
            return RC::SUCCESS;
        }
    ...
    if (page_num > file_header_->page_count) {
        STD_WARN("page %d is not continuous. file=%s, page_count=%d",
    ...
    if (file_header_->page_count >= BPFileHeader::MAX_PAGE_NUM) {
        STD_WARN("file buffer pool is full. page count %d, max page count %d",
                 file_header_->page_count, BPFileHeader::MAX_PAGE_NUM);
        return RC::INTERNAL;
    }
    ...
    Bitmap bitmap(file_header_->bitmap, file_header_->page_count);
    bitmap.set_bit(page_num);
    STD_INFO("[redo] allocate new page. file=%s, pageNum=%d", file_name_.c_str(), page_num);
    return RC::SUCCESS;
}
...
RC DiskBufferPool::redo_deallocate_page(LSN lsn, PageNum page_num) {
    ...
    Bitmap bitmap(file_header_->bitmap, file_header_->page_count);
    if (!bitmap.get_bit(page_num)) {
        STD_WARN("page %d has been deallocated. file=%s", page_num, file_name_.c_str());
        return RC::INTERNAL;
    }
    ...
    hdr_frame_->set_lsn(lsn);
    hdr_frame_->mark_dirty();
    STD_INFO("[redo] deallocate page. file=%s, pageNum=%d", file_name_.c_str(), page_num);
    return RC::SUCCESS;
}
...
RC DiskBufferPool::allocate_frame(PageNum page_num, Frame **buffer) {
    auto purger = [this](Frame *frame) {
        ...
        if (rc != RC::SUCCESS) {
            STD_ERROR("Failed to aclloc block due to failed to flush old block. rc=%s", strrc(rc));
        }
        return rc;
    };
    ...
    while (true) {
        ...
        if (frame != nullptr) {
            *buffer = frame;
            STD_INFO("allocate frame %p, page num %d", frame, page_num);
            return RC::SUCCESS;
        }

        STD_INFO("frames are all allocated, so we should purge some frames to get one free frame");
...
RC DiskBufferPool::check_page_num(PageNum page_num) {
    if (page_num >= file_header_->page_count) {
        STD_ERROR("Invalid pageNum:%d, file's name:%s", page_num, file_name_.c_str());
        return RC::BUFFERPOOL_INVALID_PAGE_NUM;
    }
...
RC DiskBufferPool::load_page(PageNum page_num, Frame *frame) {
    ...
    //期望的rc = RC::SUCCESS或者RC::BUFFERPOOL_INVALID_PAGE_NUM
    if (OB_SUCC(rc)) {
        return rc;
    }
    ...
    if (lseek(file_desc_, offset, SEEK_SET) == -1) {
        STD_ERROR("Failed to load page %s:%d, due to failed to lseek:%s.", file_name_.c_str(), page_num,
                  strerror(errno));
    ...
    if (ret != 0) {
        STD_ERROR(
                "Failed to load page %s, file_desc:%d, page num:%d, due to failed to read data:%s, ret=%d, page count=%d",
                file_name_.c_str(), file_desc_, page_num, strerror(errno), ret, file_header_->allocated_pages);
        char buf[255];
        getcwd(buf, 255);
        STD_ERROR("Working dir: %s", buf);
        return RC::IOERR_READ;
    }

    frame->set_page_num(page_num);

    STD_INFO("Load page %s:%d, file_desc:%d, frame=%s",
...
bool DiskBufferPool::is_file_empty() const {
    if (file_header_->page_count == 1) {
        return true;
    }
    return false;
}
...
BufferPoolManager::BufferPoolManager(int memory_size /* = 0 */) {
    ...
    STD_INFO("buffer pool manager init with memory size %d, page num: %d, pool num: %d",
...
RC BufferPoolManager::create_file(const char *file_name) {
    ...
    if (fd < 0) {
        STD_ERROR("Failed to create %s, due to %s.", file_name, strerror(errno));
    ...
    if (fd < 0) {
        STD_ERROR("Failed to open for readwrite %s, due to %s.", file_name, strerror(errno));
        return RC::IOERR_ACCESS;
    }
    ...
    if (lseek(fd, 0, SEEK_SET) == -1) {
        STD_ERROR("Failed to seek file %s to position 0, due to %s .", file_name, strerror(errno));
        close(fd);
        return RC::IOERR_SEEK;
    }

    if (writen(fd, (char *) &page, BP_PAGE_SIZE) != 0) {
        STD_ERROR("Failed to write header to file %s, due to %s.", file_name, strerror(errno));
        close(fd);
        return RC::IOERR_WRITE;
    }

    close(fd);
    STD_INFO("Successfully create %s.", file_name);
    return RC::SUCCESS;
}
...
RC BufferPoolManager::open_file(LogHandler &log_handler, const char *_file_name, DiskBufferPool *&_bp) {
    ...
    if (buffer_pools_.find(file_name) != buffer_pools_.end()) {
        STD_WARN("file already opened. file name=%s", _file_name);
    ...
    if (rc != RC::SUCCESS) {
        STD_WARN("failed to open file name");
        delete bp;
        return rc;
    }
    ...
    STD_INFO("insert buffer pool into fd buffer pools. fd=%d, bp=%p, lbt=%s", bp->file_desc(), bp, lbt());
    _bp = bp;
    return RC::SUCCESS;
}
...
RC BufferPoolManager::close_file(const char *_file_name) {
    ...
    if (iter == buffer_pools_.end()) {
        STD_INFO("file has not opened: %s", _file_name);
        lock_.unlock();
        return RC::INTERNAL;
    }
...
RC BufferPoolManager::flush_page(Frame &frame) {
    ...
    if (iter == id_to_buffer_pools_.end()) {
        STD_WARN("unknown buffer pool of id %d", buffer_pool_id);
        return RC::INTERNAL;
    }
...
RC BufferPoolManager::get_buffer_pool(int32_t id, DiskBufferPool *&bp) {
    ...
    if (iter == id_to_buffer_pools_.end()) {
        STD_WARN("unknown buffer pool of id %d", id);
        return RC::INTERNAL;
    }
