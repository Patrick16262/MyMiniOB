...
RC Table::create(Db *db, int32_t table_id, const char *path, const char *name, const char *base_dir,
    ...
    table_meta_.init(table_id, name, trx_fields, attributes);
...
RC Table::insert_record(Record &record) {
    LOG_INFO("Insert record into table %s", name());
    RC rc = RC::SUCCESS;
    rc = record_handler_->insert_record(record);
    if (rc != RC::SUCCESS) {
    ...
    LOG_INFO("Insert record success. table name=%s, rid=%s", table_meta_.name(), record.rid().to_string().c_str());
    return rc;
}
...
RC Table::make_record(int value_num, const Value *values, Record &record) {
    ...
    for (int i = 0; i < value_num; i++) {
        ...
        if (field->is_len_variable()) {
            // gen placeholder data
            assert(copy_len == 4);
            int len = value.length();
            memcpy(record_data  + field->offset(), &len,  copy_len);
            Record* expanded_data = generate_expanded_data(value.data(), len, record_size);
            move_to_last(record, expanded_data);
        } else {
...
void Table::get_record_scanner(RecordFileScanner &scanner) {
    scanner.open_scan(record_handler_);
}
...
RC Table::delete_record(const Record &record) {
    LOG_INFO("Delete record from table %s", name());
    RC rc = RC::SUCCESS;
    ...
    LOG_INFO("Delete record success. table name=%s, rid=%s", table_meta_.name(), record.rid().to_string().c_str());
    return rc;
}
